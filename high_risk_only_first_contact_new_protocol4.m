function [high_risk_nodes2]=high_risk_only_first_contact_new_protocol4(X_duration,X_duration_advertize,duration_threshold,covid_indexes,l22,l2,erased_indexes,unerased_indexes)
high_risk_nodes3=[];h=[];
...
for i=1:l2
    if sum(i==covid_indexes)
        if sum(i==unerased_indexes) %If it is active
            ...
            for j=1:size(X_duration_advertize{i},2)
                if X_duration_advertize{i}(1,j)>= duration_threshold
%                     if  sum(X_duration_advertize{i}(2,j)==unerased_indexes) % X_duration_advertize{i}(2,j) is active
                        high_risk_nodes3=[high_risk_nodes3 X_duration_advertize{i}(2,j)];
                        active_list=[active_list X_duration_advertize{i}(2,j)];
                        active_list_time1=[active_list_time1 X_duration_advertize{i}(3,j)];
                        active_list_time2=[active_list_time2 X_duration_advertize{i}(1,j)];
%                     end
                end
            end
